...
// const Portfolio = ({ wishlist, isLoading, onBuyStock }) => {
//   // State for controlling the modal and wishlist visibility
//   const [isModalOpen, setIsModalOpen] = useState(false);
//   const [selectedStock, setSelectedStock] = useState(null);
//   const [buyPrice, setBuyPrice] = useState('');
//   const [quantity, setQuantity] = useState(1);  // State for stock quantity
//   const [error, setError] = useState('');
//   const [isWishlistVisible, setIsWishlistVisible] = useState(true); // State to control wishlist visibility
//   const [portfolio, setPortfolio] = useState([]); // State for portfolio data
//   const [isLoadingPortfolio, setIsLoadingPortfolio] = useState(true); // State for loading state
...
//   // Function to open the modal with stock data
...
//     // Prepare the stock data to send in the POST request
//     const stockData = {
//       name: selectedStock.name,
//       ticker: selectedStock.ticker,
//       buyPrice: parseFloat(buyPrice),
//       quantity: parseInt(quantity),
//       currentPrice: selectedStock.currentPrice, // Assuming you're passing the current price
//     };

//     try {
//       // Send a POST request to save the stock in the portfolio
//       const response = await fetch('http://localhost:8080/api/stocks/portfolio', {
//         method: 'POST',
//         headers: {
...
//       if (response.ok) {
//         // On successful submission, update portfolio
//         const newStock = await response.json();
//         setPortfolio((prevPortfolio) => [...prevPortfolio, newStock]); // Add new stock to the portfolio
//       } else {
...
const Portfolio = ({ wishlist, setWishlist, isLoading, onBuyStock  }) => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        const response = await fetch('${process.env.REACT_APP_API_URL}/api/stocks/portfolio');
        if (response.ok) {
          const data = await response.json();
          setPortfolio(data);
        } else {
