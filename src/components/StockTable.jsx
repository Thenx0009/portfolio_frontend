...
const StockTable = ({ onAddToWishlist }) => {
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      try {
        const response = await fetch('https://portfolio-backend-latest-xngv.onrender.com/api/stocks');
        if (!response.ok) {
        ...
        const updatedStocks = await Promise.all(
          data.map(async (stock) => {
            const priceResponse = await fetch(
              `https://portfolio-backend-latest-xngv.onrender.com/api/stocks/real-time-price/${stock.ticker}`
            );
            if (!priceResponse.ok) {
